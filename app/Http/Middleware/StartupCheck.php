<?php namespace LeadAssurance\Http\Middleware;
...
use Event;
use Schema;
use LeadAssurance\Models\Language;
use LeadAssurance\Models\InvoiceDesign;
use LeadAssurance\Events\UserSettingsChanged;
use LeadAssurance\Libraries\CurlUtils;
...
class StartupCheck
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (!Utils::isLeadAssu() && !Utils::isDatabaseSetup()) {
            return $next($request);
        }
        ...
        if (isset($_SERVER['REQUEST_URI'])) {
            ...
            if (!$claimingLicense && Input::has('license_key') && Input::has('product_id')) {
                ...
                $url = (Utils::isLeadAssuDev() ? SITE_URL : NINJA_APP_URL) . "/claim_license?license_key={$licenseKey}&product_id={$productId}&get_date=true";
